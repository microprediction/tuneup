from tuneup.ndimhorserace import run_race
from tuneup.ndimsingleobjectives.ndimboxobjectives import OBJECTIVES, DEAP_OBJECTIVES
from tuneup.ndimsolvers.ndimboxsolvers import OPEN_SOURCE_SOLVERS, VENDOR_SOLVERS, shgo_cube, powell_cube,sigopt_cube
...
def race_specification(debug:bool):
    ...
    solvers_for_thresholds = VENDOR_SOLVERS + [shgo_cube, powell_cube]
    objectives = DEAP_OBJECTIVES
    objective_thinning = 3 if debug else 1   # e.g. if 3 we use every 3rd objective, on average.
    max_thresholds = 5 if debug else 20
    n_outer_repeat = 1000 if not debug else 5
    n_threshold_repeat = 2 if not debug else 1  # Number of times to call each solver when setting scoring scale
    n_trials = 200 if not debug else 10  # Number of evaluations of the objective function
    ...
    threshold_trials = [32, 64, 128, 256, 512][:max_thresholds]
