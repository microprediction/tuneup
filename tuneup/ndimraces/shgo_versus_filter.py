...
from tuneup.trivariateobjectives.trivariateboxobjectives import EXPNORM_OBJECTIVES
...
def race_specification(debug:bool):
    n_dim = 3 if not debug else 3
    n_trials = 100 if not debug else 5
    bad_solver = shgo_cube # Does not do the right number of function evals
    good_solvers = [optuna_cube, hyperopt_cube, pysot_cube, pattern_cube]
    ...
    n_inner_repeat = 3 if not debug else 2  # Number of times to run the horse race
